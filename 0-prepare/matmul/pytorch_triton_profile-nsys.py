...
if hasattr(matmul_kernel, 'best_config'):
    print("Best config selected by autotune:")
    print(matmul_kernel.best_config)
else:
    print("No best config found yet.")

c_ref = torch.matmul(a, b)  # 使用PyTorch的matmul作为参考

print(f"Max error: {torch.max(torch.abs(c_triton - c_ref))}")
print(f"Mean error: {torch.mean(torch.abs(c_triton - c_ref))}")

# Best config selected by autotune:
# BLOCK_SIZE_M: 128, BLOCK_SIZE_N: 128, BLOCK_SIZE_K: 32, GROUP_SIZE_M: 8, num_warps: 4, num_ctas: 1, num_stages: 4, num_buffers_warp_spec: 0, num_consumer_groups: 0, reg_dec_producer: 0, reg_inc_consumer: 0, maxnreg: None
# Max error: 0.0
# Mean error: 0.0